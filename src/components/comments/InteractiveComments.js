  ...
  const loadComments = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/articles/${articleSlug}/comments?sort=${sortBy}`,
      );
      if (response.ok) {
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        showToast(
          data.error || "Erreur lors de l'ajout du commentaire",
          "error",
        );
      }
    } catch (error) {
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center space-x-4">
                  <span
                    className={`small-text ${newComment.length > 450 ? "text-red-500" : "text-gray-500"}`}
                  >
...
function CommentItem({
  ...
  return (
    <div
      className={`bg-white rounded-lg border border-gray-200 p-6 shadow-sm transition-shadow hover:shadow-md ${isReply ? "ml-12 mt-3 border-l-4 border-l-teal-500" : ""}`}
    >
      <div className="flex space-x-4">
        {/* Avatar */}
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              ...
              <span className="small-text text-gray-500">{timeAgo}</span>
              {comment.edited && (
          ...
          <div className="mb-4">
            {isEditing ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`small-text ${editContent.length > 450 ? "text-red-500" : "text-gray-500"}`}
                  >
